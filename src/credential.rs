...
pub trait CredentialStoreTrait {
    async fn id_for(&self, session_key: &str) -> Result<String, CredentialStoreError>;
    async fn accounts(
        &self,
        session_key: &str,
    ) -> Result<HashMap<String, String>, CredentialStoreError>;
    async fn client_for(&self, session_key: &str) -> Result<ApiClient, AppError>;
    async fn auth(&mut self, owner_key: Option<String>) -> Result<(String, String), AppError>;
...
pub struct PgsqlCredentialStore {
    cm: CacheManager,
...
#[async_trait]
impl CredentialStoreTrait for PgsqlCredentialStore {
    async fn id_for(&self, session_key: &str) -> Result<String, CredentialStoreError> {
        let rec = sqlx::query!(
            r#"
